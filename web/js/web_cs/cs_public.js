...
// --------------------聊天室内页面----------------------------------------------------
	...
	$('#subxx').click(function(event) {
		...
		if(str!='') {
			...
			sends_message('美国队长', 12, str); // sends_message(昵称,头像id,聊天内容);
...
// -----下边的代码不用管---------------------------------------
	...
	function sends_message (userName, userPortrait, message) {
		if(message!='') {
			$('.main .chat_info').html($('.main .chat_info').html() + '<li class="right"><img src="/images/user/' + userPortrait + '.png" alt=""><b>' + userName + '</b><i>' + getTime() + '</i><div class="aaa">' + message  +'</div></li>');
		}
	}

	function receive_message(userName, userPortrait, message) {
		$('.main .chat_info').html($('.main .chat_info').html() + '<li class="left"><img src="/images/user/' + userPortrait + '.png" alt=""><b>' + userName + '</b><i>' + getTime() + '</i><div class="aaa">' + message  +'</div></li>');
	}

	function receive_system_message(message) {
		$('.main .chat_info').html($('.main .chat_info').html() + '<li class="systeminfo"><span>' + message + '</span></li>');
	}
